...
| 升级项 | 内容 |
|---|---|
| **短线冲高选股** | 新增 `docs/14_短线冲高选股与交易方法论.md` + `skills/chip-reversal-stock-picker/`：筹码分歧回补6条件 + 四维度共振分级 + 竞价抢筹 + 主力成本线 + 健康吸筹/高位派发镜像判据（中天科技实战） |
| **A股抖音口播** | 新增 `docs/15_A股抖音口播方法论.md` + `skills/a-share-douyin-koubo/`：五框架（流动性档位/通胀剪刀差/冲锋号/跨境支付/一日五步）+ 三模板（盘前/午间/盘后）+ 写作纪律 |
| **报告输出规范** | 新增 `docs/16_报告输出规范_副文本与涨停池排版.md`：副文本优先铁律 + 涨停池排版范式 + 两条铁律（完整全量/板块强度排序）+ 涨停数据来源 |
| **情绪周期校准** | `memory/emotion_cycle.md` 补充 9 月涨停序列：9/8 73 → 9/15 32(冰点) → 9/16 91(冰点反转) → 9/21 105(高潮无高度·首板扩散) |

> 注：v3.4 为「三系统融合决策方法论」（docs/13），本次 v3.5 在其基础上新增短线选股与内容产出两条新能力线。
